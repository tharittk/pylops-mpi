...
def _prepare_buffers_allgather(send_buf, send_buf_shapes) -> (cp.ndarray, cp.ndarray):
    """ Preparing the send_buf and recv_buf for the NCCL allGather 
...
def _condition_recv_allgather(recv_buf, padded_send_buf_shape, send_buf_shapes) -> list:
...
def nccl_asarray(nccl_comm, local_array, local_shapes, axis) -> cp.ndarray:
    ...
    # ndev = len(local_shapes)
    # recv_buf = cp.zeros(ndev * send_buf.size, dtype=send_buf.dtype)
    (send_buf, recv_buf) = _prepare_buffers_allgather(local_array, local_shapes)
    ...
    chunks = _condition_recv_allgather(recv_buf, send_buf.shape, local_shapes) 
